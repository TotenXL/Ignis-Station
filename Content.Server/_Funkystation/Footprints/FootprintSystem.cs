...
public sealed partial class FootprintSystem : EntitySystem
{
    ...
    [Dependency] private StandingStateSystem _standing = null!;
    ...
    private void OnEntityMoved(EntityUid uid, FootprintOwnerComponent component, ref MoveEvent args)
    {
        ...
        var isStanding = !_standing.IsDown(uid);
    ...
    private bool ProcessPuddleStepping(EntityUid uid, FootprintOwnerComponent component, EntityUid gridUid, MapGridComponent grid, Vector2i tile, bool isStanding)
    {
        ...
        var ownerSolution = EnsurePrintSolution(uid, FixedPoint2.Max(component.MaxFootVolume, component.MaxBodyVolume));
    ...
    private void CreateFootprint(EntityUid uid, FootprintOwnerComponent component, EntityUid gridUid, MapGridComponent grid, Vector2i tile, EntityCoordinates coords, Angle rotation, bool isStanding)
    {
        ...
        var printSolution = EnsurePrintSolution(printUid, MaxVolumePerTile);

        var maxVol = isStanding ? component.MaxFootprintVolume : component.MaxBodyprintVolume;
        var alpha = Math.Clamp((float)transferAmount / maxVol * component.PrintOpacity, 0f, 1f);
    ...
    /// <summary>
    /// Fetches the print solution of an entity, creating it (with at least the given capacity) if it doesn't exist yet.
    /// </summary>
    private Entity<SolutionComponent> EnsurePrintSolution(EntityUid uid, FixedPoint2 capacity)
    {
        if (!_solutionContainer.TryGetSolution(uid, PrintSolutionName, out var existing, out _))
        {
#pragma warning disable CS0618 // Named solutions are still the only way to do this.
            _solutionContainer.EnsureSolution(uid, PrintSolutionName, out var created);
#pragma warning restore CS0618
            _solutionContainer.SetCapacity(created, capacity);
            return created;
        }

        if (existing.Value.Comp.Solution.MaxVolume < capacity)
            _solutionContainer.SetCapacity(existing.Value, capacity);
    ...
    {
        var enumerator = _map.GetAnchoredEntities(gridUid, grid, tile);
